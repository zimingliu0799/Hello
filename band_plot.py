...
mpl.use('Agg') #silent mode            #仅保存不打印，需在调用pyplot前使用
...
df=np.loadtxt("REFORMATTED_BAND.dat",dtype=np.float64)
...
axe = plt.subplot(111)
axe.plot(df[:,0],df[:,1:],linewidth=1.5,color='red')
font = {'family' : 'arial',  
        'color'  : 'black',  
        'weight' : 'normal',  
        'size'   : 13,  
...
axe.axhline(y=0, xmin=0, xmax=1,linestyle= '--',linewidth=0.7,color='0.5')
for i in x[1:-1]:
	axe.axvline(x=i, ymin=0, ymax=1,linestyle= '--',linewidth=0.7,color='0.5')
axe.set_xlim((x[0], x[-1]))
plt.ylim((-1.5, 1.5)) # set y limits manually
fig = plt.gcf()
